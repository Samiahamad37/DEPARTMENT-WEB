...
import { ArrowLeft, Calendar, User, Tag, Clock, Eye, Newspaper, Share2, Heart, MessageCircle, Bookmark } from 'lucide-react';
...
const NewsDetails: React.FC = () => {
  ...
  const getCategoryColor = (category: string) => {
    const colors: { [key: string]: string } = {
      ...
      student_achievement: 'bg-yellow-100 text-yellow-800',
      faculty_news: 'bg-indigo-100 text-indigo-800',
      department_news: 'bg-orange-100 text-orange-800',
        ...
        {news.image && (
          <div className="mb-8 relative group">
            <img
              src={news.image}
              alt={news.title}
              className="w-full h-64 lg:h-96 object-cover rounded-lg shadow-lg transition-transform duration-300 group-hover:scale-105"
            />
        ...
        {relatedNews && relatedNews.length > 0 && (
          <div className="mt-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {relatedNews.results?.filter(n => n.id !== news.id).slice(0, 3).map((relatedArticle) => (
                <div key={relatedArticle.id} className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow">
                  ...
                  <div className="p-6">
                    <div className="flex items-center justify-between mb-2">
                      <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${getCategoryColor(relatedArticle.category)}`}>
                    ...
                    <h3 className="font-semibold text-gray-900 mb-2 line-clamp-2">{relatedArticle.title}</h3>
                    <p className="text-gray-600 text-sm line-clamp-3 mb-4">{relatedArticle.description}</p>
                    <div className="flex items-center justify-between">
                      <span className="text-xs text-gray-500">{formatDate(relatedArticle.date_posted)}</span>
                      <button
